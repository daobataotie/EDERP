    ...
    public partial class StockAccessor : EntityAccessor, IStockAccessor
    {
        ...
        public DataTable SelectProductNoDepotout(double years, string productCategoryId)
        {
            ...
            if (string.IsNullOrEmpty(productCategoryId))
                ...
                sql = "select * from (select a.ProductVersion,a.productid,a.ProductName,a.CustomerProductName,a.StocksQuantity,a.CustomerShortName, (case when isnull(a.LastDepotoutDate,0)>isnull(a.LastDepotoutDate2,0) then a.LastDepotoutDate else a.LastDepotoutDate2 end) as LastDepotoutDate  from (SELECT p.ProductVersion,p.productid,p.ProductName,p.CustomerProductName,p.StocksQuantity,c.CustomerShortName,(select top 1 DepotOutDate from DepotOut where DepotOutId in (select DepotOutId from DepotOutDetail where ProductId=p.ProductId) order by DepotOutDate  desc) as LastDepotoutDate ,(select top 1 InvoiceDate from InvoiceXS where InvoiceId in (select InvoiceId from InvoiceXSDetail where ProductId=p.ProductId) order by InvoiceDate desc) as LastDepotoutDate2 FROM product p left join Customer c on p.CustomerId=c.CustomerId where StocksQuantity<>0) a) b where DATEDIFF(dd,b.LastDepotoutDate,GETDATE())>" + Years + " or b.LastDepotoutDate is null";
            else
                ...
                sql = "select * from (select a.ProductVersion,a.productid,a.ProductName,a.CustomerProductName,a.StocksQuantity,a.CustomerShortName, (case when isnull(a.LastDepotoutDate,0)>isnull(a.LastDepotoutDate2,0) then a.LastDepotoutDate else a.LastDepotoutDate2 end) as LastDepotoutDate  from (SELECT p.ProductVersion,p.productid,p.ProductName,p.CustomerProductName,p.StocksQuantity,c.CustomerShortName,(select top 1 DepotOutDate from DepotOut where DepotOutId in (select DepotOutId from DepotOutDetail where ProductId=p.ProductId) order by DepotOutDate  desc) as LastDepotoutDate ,(select top 1 InvoiceDate from InvoiceXS where InvoiceId in (select InvoiceId from InvoiceXSDetail where ProductId=p.ProductId) order by InvoiceDate desc) as LastDepotoutDate2 FROM product p left join Customer c on p.CustomerId=c.CustomerId where StocksQuantity<>0 and p.ProductCategoryId='" + productCategoryId + "') a) b where DATEDIFF(dd,b.LastDepotoutDate,GETDATE())>" + Years + " or b.LastDepotoutDate is null";
            DataTable dt = new DataTable();
