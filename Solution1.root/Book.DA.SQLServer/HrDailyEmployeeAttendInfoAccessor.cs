...
namespace Book.DA.SQLServer
{
    ...
    public partial class HrDailyEmployeeAttendInfoAccessor : EntityAccessor, IHrDailyEmployeeAttendInfoAccessor
    {
        ...
        public DataSet SelectByEmpMonth(DateTime date)
        {
            ...
            SqlConnection con = new SqlConnection(sqlmapper.DataSource.ConnectionString);
            StringBuilder sqlBuilder = new StringBuilder();
            sqlBuilder.Append("SELECT e.EmployeeName,'('+ CASE datepart(weekday,o.DutyDate) WHEN 1 THEN '日' WHEN 2 THEN '一' WHEN 3 THEN '二' WHEN 4 THEN '三' WHEN 5 THEN '四'	WHEN 6 THEN '五' WHEN 7 THEN '六' END+')' weekofday,HrDailyEmployeeAttendInfoId,ClockDataId,o.EmployeeId,DutyDate,ShouldCheckIn,ShouldCheckOut,isnull( CONVERT(varchar(10), Replicate('0',2-len(datepart(hh ,ActualCheckIn)))+convert(varchar(2), datepart(hh ,ActualCheckIn))+':'+ Replicate('0',2-len(datepart(n ,ActualCheckIn)))+convert(varchar(2), datepart(n ,ActualCheckIn))),'--:--') ActualCheckIn,isnull( CONVERT(varchar(10), Replicate('0',2-len(datepart(hh ,ActualCheckOut)))+convert(varchar(2), datepart(hh ,ActualCheckOut))+':'+  Replicate('0',2-len(datepart(n ,ActualCheckOut)))+convert(varchar(2), datepart(n ,ActualCheckOut))),'--:--')   ActualCheckOut,LateInMinute,isnull(DayFactor,0) DayFactor,MonthFactor,SpecialBonus,o.Note,   IsNormal,isnull(t.EoverTime,0) AS EoverTime,e.EmployeeJoinDate,	(SELECT Fromtime FROM BusinessHours WHERE e.BusinessHoursId = BusinessHours.BusinessHoursId) AS Fromtime,(SELECT ToTime FROM BusinessHours WHERE e.BusinessHoursId = BusinessHours.BusinessHoursId) AS ToTime,(select DepartmentName from Department where DepartmentId=e.DepartmentId) as DepartmentName FROM HrDailyEmployeeAttendInfo o left JOIN Employee e ON e.EmployeeId = o.EmployeeId left JOIN OverTime t ON  Convert(varchar,t.DueDate,23)=convert(varchar,o.DutyDate,23) and t.EmployeeId=o.EmployeeId where 1=1 ");
            if (date != null)
                sqlBuilder.Append(" and   year(o.DutyDate)=" + year + " and month(o.DutyDate)=" + month + "");
        ...
        public System.Data.DataTable SelectHrInfoByStateAndDate(DateTime DutyDate)
        {
            ...
            //当考勤日期为国定假日，且员工有请假，那么考勤Note记录的是国假名称，以“a.Note not like '%'+lt.LeaveTypeName+'%'”判断就会出异常考勤
            string sqlStr = "select a.HrDailyEmployeeAttendInfoId, a.DutyDate,b.EmployeeId,b.EmployeeName,b.IDNo,a.ShouldCheckIn,a.ShouldCheckOut,a.ActualCheckIn,a.ActualCheckOut,a.Note, (case when  a.Note<>'' then a.Note else lt.LeaveTypeName end) as Note,a.DayFactor,a.MonthFactor,a.LateInMinute,d.DepartmentName  from  HrDailyEmployeeAttendInfo as a inner join Employee as b on a.EmployeeId=b.EmployeeId left join Department d on b.DepartmentId=d.DepartmentId left join Leave l on l.EmployeeId=b.EmployeeId and l.LeaveDate=a.DutyDate left join LeaveType lt on l.LeaveTypeId=lt.LeaveTypeId where (a.IsNormal=0 or a.Note like '%曠職%' or (a.Note not like '%'+lt.LeaveTypeName+'%'  and a.Note<>(select HolidayName from AnnualHoliday where HolidayDate='" + DutyDate.ToString("yyyy-MM-dd") + "'))) and DutyDate='" + DutyDate.ToString("yyyy-MM-dd") + "'  and left(b.IDNo,1)<>'k' ORDER BY (case when left(idno,1) like '[A-Za-z]' then (case when convert(int,substring(idno,2,2)) between 30 and 99 then left(idno,1)+cast(1911+convert(int,substring(idno,2,2)) as varchar(10))+substring(idno,4,len(idno)) else left(idno,1)+convert(varchar(10),1911+convert(int,'1'+substring(idno,2,2)))+substring(idno,4,len(idno)) end ) else idno end) ";
