...
namespace Book.DA.SQLServer
{
    ...
    public partial class InvoiceXODetailAccessor : EntityAccessor, IInvoiceXODetailAccessor
    {
        ...
        //客户出货表
        public DataTable ShipmentTable(string customerIds, DateTime startDate, DateTime endDate)
        {
            string sql = "select a.*,p.CustomerProductName,p.NetWeight,p.GrossWeight,p.Volume,a.InvoiceXODetailQuantity/ISNULL(p.Xiangzhuang,1) as BoxCount,(Cast(ISNULL(p.Hezhuang,0) as varchar(10))+pu.CnName+'/'+Cast(ISNULL(p.Xiangzhuang,0) as varchar(20))+pu.CnName) as Guige,(select Cast(Workhousename as varchar(20)) from WorkHouse where WorkHouseId in (select WorkHouseId from PronoteProceduresDetail where PronoteHeaderID in (select PronoteHeaderID  from PronoteHeader where InvoiceXOId=a.InvoiceId and ProductId=a.ProductId)) for xml path('')) as Workhousename,'' as BoxNumber,pu.CnName as SellUnit from (select  c.CustomerFullName,xo.CustomerInvoiceXOId,xod.ProductId,Convert(varchar(20),xo.InvoiceYjrq,23) as InvoiceYjrq,Sum(xod.InvoiceXODetailQuantity) as InvoiceXODetailQuantity,xo.InvoiceId from InvoiceXODetail xod left join InvoiceXO xo on xo.InvoiceId=xod.InvoiceId left join Customer c on xo.xocustomerId=c.CustomerId where c.CustomerId in (" + customerIds + ") and xo.InvoiceYjrq between '" + startDate.ToString("yyyy-MM-dd") + "' and '" + endDate.Date.AddDays(1).AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "' group by c.CustomerFullName,xo.CustomerInvoiceXOId,xod.ProductId,xo.InvoiceYjrq,xo.InvoiceId) a left join Product p on a.ProductId=p.ProductId left join ProductUnit pu on p.SellUnitId=pu.ProductUnitId where p.CustomerProductName<>'' order by a.InvoiceYjrq,a.CustomerFullName,a.CustomerInvoiceXOId,p.CustomerProductName";
